...
function findUsernameField(container, passwordField) {
  ...
  for (const selector of usernameSelectors) {
    ...
    for (const field of fields) {
      if (
        field.compareDocumentPosition(passwordField) &
...
/**
 * Create and show blocked autofill badge
 */
...
function showSavePasswordPrompt(username, password, domain, isUpdate = false) {
  // TODO: fix issue with it showing up even when already exists
...
function createAndShowPrompt(username, password, domain, isUpdate = false) {
  ...
  prompt.innerHTML = `
    <div class="prompt-content">
      ...
      } for <strong>${new URL(domain).hostname}</strong>?</p>
      ...
      mutation.removedNodes.forEach((node) => {
        if (node === prompt && prompt.dataset.userDismissed !== "true") {
          ...
          setTimeout(() => {
            if (
              document.body &&
              !document.getElementById("opaque-save-prompt")
            ) {
              document.body.appendChild(prompt);
            }
          }, 100);
  ...
  saveBtn.addEventListener("click", async () => {
    try {
      ...
      // Clear the dismissal flag since user chose to save
  ...
  neverBtn.addEventListener("click", () => {
    ...
    // Clear the dismissal flag since this is permanent
    chrome.storage.local.remove([`opaque_prompt_dismissed_${domain}`], () => {
      if (chrome.runtime.lastError) {
        console.error(
          "[Autofill] Failed to clear dismissal flag:",
  ...
  notNowBtn.addEventListener("click", () => {
    prompt.dataset.userDismissed = "true"; // Mark as user action
    promptObserver.disconnect(); // Stop observing
    // Store dismissal in extension storage (persists across page refreshes)
    chrome.storage.local.set(
...
async function saveToDraft(username, password) {
  ...
  // Check if credentials already match stored ones (need to await this)
  const credentials = await getStoredCredentials(domain);
  if (
    credentials &&
    ...
    signInBtn.addEventListener("click", async () => {
      ...
      try {
        ...
        if (credentials && credentials.username && credentials.password) {
          ...
          const success = await performOpaqueLogin(
...
function setupFocusDetection() {
  ...
  document.addEventListener("focusin", (event) => {
    ...
    if (isCredentialField) {
      console.log(
        "[Autofill] Credential field focused, checking for autofill opportunity"
      );
...
async function checkForDraftAndPrompt() {
  ...
  if (draft && draft.username && draft.password && credential === null) {
    ...
    // Check if user dismissed the prompt (stored in extension storage)
...
async function attemptAutofill() {
  ...
  if (!nonceAllowed) {
    ...
    // If we *do* have credentials or a draft, offer popup
    const domain = getCurrentDomain();
    const credentials = await getStoredCredentials(domain);
    const draft = getDraft?.();
