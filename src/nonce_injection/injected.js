...
//  - Handle modern login layouts where <form> may not exist.
...
(function () {
  ...
  /**
   * Generate a 16-byte hex nonce.
   * Uses crypto when available; fallback is less ideal but acceptable.
   */
  ...
  /**
   * Fallback method for finding a submit control in non-form login UIs.
   * Attempts to identify the most likely "submit" element.
   */
  ...
  /**
   * Apply nonces to all login targets found.
   * Returns the number of newly processed elements.
   */
  function injectNonceIntoTargets() {
    ...
    targets.forEach((target, index) => {
      ...
      if (!element || inputs.length === 0) return;

      if (element.dataset.nonce) return;
      ...
      const selector =
        element.id && type === "form"
          ...
          : `${element.tagName.toLowerCase()}[index=${index}]`;
  ...
  /**
   * Observe DOM changes to catch dynamically created login components.
   * Important for modern SPAs and interactive login interfaces.
   */
  function setupMutationObserver() {
    try {
      const observer = new MutationObserver((mutations) => {
          ...
          if (mutation.type === "childList") {
            mutation.addedNodes.forEach((node) => {
              if (node.nodeType === Node.ELEMENT_NODE) {
                ...
                if (
                  el.matches?.('input[type="password"], form') ||
                  el.querySelector?.('input[type="password"], form')
                ) {
                  relevantChange = true;
                }
              }
        ...
        if (relevantChange) injectNonceIntoTargets();
      });
