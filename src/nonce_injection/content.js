...
// Content script for nonce injection.
//
//  - Load injected.js into the real page environment.
//  - Maintain a small in-memory registry of nonces (per tab).
//  - Mark nonces as "used" when submission events occur.
//  - Provide quick integrity checks for autofill logic.

console.log("[NonceInjection][Content] Content script loaded");

// Format: Map<nonceString, { issuedAt: number, used: boolean }>
const nonceRegistry = new Map();

/**
 * Inject injected.js into the page so it can interact directly with page JS.
 */
function injectPageScript() {
  try {
    ...
    script.onload = () => {
      ...
      script.remove(); // avoid DOM clutter
    };
...
/**
 * Small temporary banner confirming injection occurred.
 * Mainly used for quick visual debugging.
 */
function showSuccessBanner(formCount) {
  try {
    if (document.getElementById("nonce-injection-banner")) return;

    const banner = document.createElement("div");
    banner.id = "nonce-injection-banner";
    banner.textContent = `Nonce injection successful (${formCount} form${
      formCount === 1 ? "" : "s"
    } processed)`;
...
/**
 * Listen for notifications from injected.js.
...
function setupPageMessageListener() {
  window.addEventListener("message", (event) => {
    ...
    if (!data || data.source !== "NonceInjection") return;
    ...
    switch (data.type) {
      ...
      case "INJECTION_COMPLETE": {
        ...
        if (data.formCount > 0) showSuccessBanner(data.formCount);
        break;
      }
...
/**
 * Respond to autofill integrity checks:
 * Required output: whether at least one fresh (unused) nonce exists.
 */
chrome.runtime.onMessage.addListener((request, sender, sendResponse) => {
  ...
  if (request.action === "checkNonceStatus") {
    ...
    const summary = {
      ...
      freshNonces: [...nonceRegistry.entries()]
        .filter(([_, e]) => !e.used)
        .map(([nonce]) => nonce),
      usedNonces: [...nonceRegistry.entries()]
        .filter(([_, e]) => e.used)
        .map(([nonce]) => nonce),
    };
...
/**
 * Initialiser.
 * Injects page script and attaches message listeners.
 * Always executed, even before password fields exist (important for SPAs).
 */
function init() {
